...
- **Agent-first design** — every tool returns a Pydantic model. **237 LLM-callable tools** with OpenAI/Anthropic function-calling schemas, split across **ten parallel runtimes** whose dispatch tables refuse what they do not own, so a tool an agent was never given is unroutable rather than merely unlikely. Bulk values cross between runtimes as typed references, never through the conversation. Unknown arguments are rejected rather than silently ignored, every Sharpe-reporting tool takes the risk-free rate it is measured against, and errors are written to be self-correcting rather than merely accurate.
- **A client is served one runtime, not the whole surface** — `sqt-mcp --runtime research` costs 61 KB, and there is no session ceiling -- `CONTEXT_CEILING_BYTES` is `None`, because a cap was the wrong control, and the heaviest runtime, `modeling`, is 171 KB at full detail — there is no fixed per-runtime cap, because what a client can afford is a property of the client. `--tool-detail auto` is the DEFAULT: it thins the most expensive schemas until a runtime fits its budget and injects `describe_tool` so the full schema is one call away rather than gone — which brings `modeling` from 184 KB served to 43 KB, `backtest` from 93 to 42 and `research` from 71 to 45. Those are SERVED bytes — the envelope the server actually sends — while the per-runtime table above and `--print-budget` report bare schema bytes, so the two are not the same instrument and are not comparable line for line. The whole surface at full detail is 541 KB of schema (~138k tokens), which does not fit and is not meant to — that constraint is what the scoping exists to answer, not a regression. The schema figures are what `sqt-mcp --print-budget` reports; the served figures come from the server's own `context_bytes()`.

| runtime | tools | schema cost | what it answers |
|---|---:|---:|---|
| `research` | 42 | 61 KB | what is this asset, and what is its statistical structure |
| `data` | 21 | 33 KB | get the bytes, and say what they can and cannot support |
| `backtest` | 35 | 86 KB | does this strategy work, and how much of it is real |
| `modeling` | 37 | 171 KB | build, fit and score a model — one ordered pipeline |
| `portfolio` | 19 | 35 KB | turn a view into a position and price what it costs |
| `derivatives` | 12 | 21 KB | what an option is worth and what holding it does to you |
| `delta_one` | 18 | 43 KB | which instrument is the cheapest way to hold this exposure |
| `microstructure` | 17 | 30 KB | what the market will charge you to trade |
| `meta` | 25 | 24 KB | what does this library hold, and what did this session do |
| `feature_lab` | 11 | 38 KB | what each feature measures and predicts, before fitting |

- **Tested in nine layers, and the layers earn their place** — correctness against PLANTED answers rather than recorded outputs (second-order greeks against central finite differences, put-call parity against the model's own prices, risk parity against its closed form); whole-surface invariants that catch a tool registered halfway; schema-synthesized adversarial fuzzing that found six real bugs on its first run; metamorphic relations that catch a function which is *consistently* wrong; determinism checks that catch a `seed` being silently ignored; a second oracle for every C++ kernel, because a parity test cannot catch two backends agreeing on the wrong answer and one of them did not; generated documentation that cannot go stale; a whole-suite run with the compiled extension switched off, because a fallback that only exists behind a monkeypatch is not covered and is easily mistaken for dead code; and mutation testing, because a passing suite proves the tests RUN and not that they would notice. 8,888 tests; 21 of 21 mutations killed. See [Documentation/25_testing.md](Documentation/25_testing.md).
...
- **Audited for correctness** — both tiers have been through line-by-line audits. 41 findings in the core, then two reviews of the modeling runtime: the first found 7 critical issues (two leakage channels, a PCA start-vector degeneracy), the second found 20 more across modeling, the data layer and the numerics — a full-refit model that had seen prices past its own recorded cutoff, an `end_date` that meant different things per provider, a "cross-section" that could mix dates, and aliases that could forge feature provenance. Since then a portfolio/screener/agent-tools audit, a full-codebase pass, and the live-data pass of 2026-09-20 — the first run against a real vendor tape, which found twenty defects no synthetic fixture could show — take the record to ten sections. None of it was catchable by the suite as it stood. Every finding was reproduced against a live interpreter before being fixed, and each is pinned by a regression test. The full record, including what each defect broke: **[Documentation/17_correctness.md](Documentation/17_correctness.md)**.
...
|---|---|---|
| `data` | Provider-agnostic OHLCV, fundamentals and metadata, with caching, retries and quality checks — plus a Databento path for L2 depth, quotes, trades and order-by-order events | [01](Documentation/01_data_fetching.md), [11](Documentation/11_data_quality.md) |
...
| `metrics` | Returns, risk, drawdown, ratios, and three range-based volatility estimators beside close-to-close | [03](Documentation/03_metrics.md) |
...
| `agent` | The LLM surface across ten runtimes, its dispatch, the handoff interconnect, and orchestration patterns | [07](Documentation/07_agent_tools.md), [09](Documentation/09_advanced_agent_tools.md), [13](Documentation/13_agent_orchestration.md), [19](Documentation/19_runtimes.md) |
| `modeling` | A separate 37-tool runtime for building walk-forward-validated statistical models — from this library's own features or a feature matrix computed elsewhere and registered without copying it — plus lagged feature windows, model ensembles, error-and-calibration diagnostics, registry browsing, pre-flight spec validation, lifecycle promotion and drift monitoring | [15](Documentation/15_modeling.md), [29](Documentation/29_modeling_reference.md) |
| `audit` | Content-addressed decision records, replay verification, and the `sqt` CLI | [10](Documentation/10_auditability.md) |
| `agent.runtimes.data` | The `data` RUNTIME: fetch OHLCV, returns panels, tick tapes and quotes, build continuous futures, and publish them as `sqt://` references every other runtime reads | [26](Documentation/26_data.md) |
| `delta_one` | Carry, basis, futures curves and rolls, hedge sizing, baskets, ETF fair value, swaps and TRFs — which instrument is the cheapest way to hold an exposure | [28](Documentation/28_delta_one.md) |
| `agent.runtimes.derivatives` | Option pricing, greeks, smile and term structure, parity, delta-hedge simulation | [21](Documentation/21_derivatives.md), [12](Documentation/12_options.md) |
| `agent.runtimes.microstructure` | Spreads measured from ticks, the bar-based estimators for when there is no tick feed, and depth-book statistics | [22](Documentation/22_microstructure.md) |
| `agent.runtimes.feature_lab` | Feature profiling, drift, regime stability and permutation testing | [15](Documentation/15_modeling.md) |
...
| `Documentation/07_agent_tools.md` | Core 14 LLM tools, the full 189-tool analysis registry, Pydantic models, end-to-end agent loop |
...
| `Documentation/14_polars_support.md` | Optional Polars interop (`pip install standard_quant_tools[polars]`): what's supported today, the conversion-boundary design, and what is not adapted yet |
| `Documentation/15_modeling.md` | The separate 37-tool modeling runtime: feature catalog and feature report, regression/classification/ranking/survival targets, leakage-purged walk-forward validation, sample weighting, the model adapters, point-in-time joins, the content-addressed model registry with signed manifests, lifecycle stages and drift monitoring, the model→backtest bridge, portfolio evaluation of OOS predictions, and what's explicitly deferred |
...
| `Documentation/20_tool_index.md` | **Generated** index of all 237 tools by runtime, with the description the model sees and every argument name |
...
| `Documentation/29_modeling_reference.md` | **Generated** from the modeling registries: every feature, estimator (with bounded parameters and capabilities), target and spec option; a test fails when it drifts |

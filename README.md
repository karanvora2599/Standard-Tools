...
- **Agent-first design** — every tool returns a Pydantic model. **95 LLM-callable tools** with OpenAI/Anthropic function-calling schemas, split across **six parallel runtimes** whose dispatch tables refuse what they do not own, so a tool an agent was never given is unroutable rather than merely unlikely. Bulk values cross between runtimes as typed references, never through the conversation. Unknown arguments are rejected rather than silently ignored, every Sharpe-reporting tool takes the risk-free rate it is measured against, and errors are written to be self-correcting rather than merely accurate.
- **A client is served one runtime, not the whole surface** — `sqt-mcp --runtime research` costs 32 KB against a 176 KB session ceiling, and `--tool-detail auto` thins the most expensive schemas until a runtime fits its budget, fetching them on demand instead. Serving all 95 tools eagerly no longer fits, which is the constraint the scoping exists to answer rather than a regression.
...
| `agent` | The LLM surface across six runtimes, its dispatch, the handoff interconnect, and orchestration patterns | [07](Documentation/07_agent_tools.md), [09](Documentation/09_advanced_agent_tools.md), [13](Documentation/13_agent_orchestration.md), [19](Documentation/19_runtimes.md) |

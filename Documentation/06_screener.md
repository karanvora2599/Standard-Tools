...
**A beta that cannot be estimated is an error, not a value.** `calculate_beta` returns `alpha`/`beta`/`r_squared` all `0.0` when fewer than two points overlap the benchmark. That sentinel is indistinguishable from a real answer, because 0.0 is also a legitimate beta — and the screener *filtered* on it, so a ticker whose history did not overlap SPY at all reported beta 0.0 and **passed** `beta_max=0.5`. "Could not be estimated" was read as "very low beta", which is backwards for the defensive screen that bound exists to express. The screener now requires at least `_MIN_BETA_OBS` (20) overlapping bars and reports a shortfall as a per-ticker error. Twenty is a deliberate floor rather than the bare minimum of two: this module makes an include/exclude *decision* from beta, and a beta fitted to a handful of bars is noise either way.

...
Everything above concerns the 178-tool analysis and backtest surface. There
...
207-tool whole surface. The example implementations keep the same
...
| Size | 178 tools, 13 categories, 8 runtimes | 20 tools, one ordered pipeline |
...
run_agent(..., registry="analysis")     # 178 tools, dispatch  (the default)
```
...
single source of truth: every one of the 178 tool names mapped to exactly

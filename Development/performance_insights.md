...
| 26 | Modeling feature preprocessing (`fit_preprocess_stats` / `apply_preprocess_stats`) | ✅ IMPLEMENTED | Measured: fit **5.5–23.5×**, apply **14.5–53.6×**. Preprocessing was 47–56% of a walk-forward run — more than the estimator fit and every metric combined. The hard part was not speed: reproducing pandas' conventions (linearly interpolated quantiles at `h=(n−1)q`, ddof=1, NaN skipped by moments but kept by transforms, infinities not missing) is most of the implementation, and a **sequential accumulator disagreed with numpy's pairwise summation in the 12th significant digit** on return-scale data until the kernel was changed to sum pairwise too. |
| 27 | Per-date statistics (`cross_sectional_correlation`, `standardize_by_date`) | ✅ IMPLEMENTED | Measured: cross-sectional IC **4.9–6.2×** spearman / **3.0–4.2×** pearson, standardize **8.6–11.6×**, pooled rank IC **1.6–3.0×**. The pooled rank IC was **not in the plan** and turned out to be the largest single metric cost (41% of `regression_metrics` at 25k rows, 51% at 250k) — visible only on re-measuring *between* phases, which is the lesson worth keeping. One kernel serves both the pooled and per-date forms, since they are the same computation over different segmentations. |
| 28 | Label-overlap weights (`label_uniqueness`) | ✅ IMPLEMENTED | Measured: **8–23×** against a predicted 20–50×. First version was **0.4×** — *slower* — on a 12,600-row panel, because three pandas round trips to normalize datetimes cost more than the Python loop saved. Fixed by reinterpreting `datetime64[ns]` to int64 for free and gating the kernel below 50,000 rows. A fast path that is slower is a bug, not a trade-off, and it is only found by benchmarking the small case as carefully as the large one. |
...
| 23 | Deep native optimization, item L: runtime ISA dispatch demo (AVX2+FMA, `rolling_beta` only, scoped deliberately to one kernel) | ✅ IMPLEMENTED, tolerance-gated (gate passed) | Measured (min of 15, real dispatch vs. test-forced scalar path): **n=2000/window=60: ~1.50×**; **n=20000/window=60: ~1.10×** |
...
- **The pandas object model itself, when the profile is flat.** Measured on the
  modeling layer after its kernels landed: `_predict_fold` 34%,
  `_preprocess` 13%, `DataFrame.__getitem__` 13%, the final refit 8%,
  `estimator.fit` 6%, the parquet write 5% — and the top entries by self-time
  are `isinstance`, `Series.__init__` and `__finalize__`, spread across
  thousands of small calls. There is no hot loop left to port. **70%** of that
  run is fold slicing, DataFrame construction and I/O. A kernel removes a
  numeric loop; it does not remove a `Series` constructor. This is the single
  most useful thing to check before starting: if the profile is flat, the
  answer is a different data structure, not a faster inner loop.

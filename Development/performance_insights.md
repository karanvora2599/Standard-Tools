...
| 2 | RSI, ADX, Parabolic SAR | ✅ IMPLEMENTED | Measured vs. *warm numba* (functional on this machine): RSI 5.3×, ADX 0.9× (tied), PSAR 1.1× (tied) — see "Real benchmark results" below for why this differs sharply from the original 10–30× projection |
...
| 5 | Backtest kernel (`run_strategy` — equity + all metrics in one C++ pass) | ✅ IMPLEMENTED | Measured: ~1.0× end-to-end (n=2000) — the raw kernel is faster (confirmed by `bench_backtest.cpp`'s native-only numbers), but pandas/DataFrame wrapper overhead dominates wall time at this size; see "Real benchmark results" below |
| 6 | Batch backtest grid (`batch_run_strategy` — all combos in one C++ call) | ✅ IMPLEMENTED AND VERIFIED — win_rate/profit_factor/num_trades/avg_trade_return_pct previously used the native kernel's own uncorrected trade-log accounting; `backtest.cpp`'s native trade-log logic was rewritten 2026-07-24 to match `_build_trade_log` exactly, closing the divergence at the source. **Verified correct** by actually building `_sqt_core` and running `TestNativeTradeStatsCorrectness` plus the full native `ctest` suite — the fix was already right; 4 of the *test file's own* hand-written expectations were wrong instead (fixed, see the Executive Summary's bug list) | 10–50× vs per-combo C++ calls (not independently re-measured this pass; the batch-vs-per-combo comparison wasn't part of the real benchmark run) |
...
| 11 | Monte Carlo forward simulation (`simulate_forward_paths` — moving-block bootstrap, optional OpenMP) | ✅ IMPLEMENTED | Measured: 2.0× serial (n_simulations=5000, horizon=60) vs. the previously-uncompiled Python loop; OpenMP adds a further ~1.4× on this 16-core machine (405.8ms→293.4ms at n_simulations=200000) — real, but not the near-linear-with-cores scaling the per-path independence would suggest in theory (see "Real benchmark results" below for why) |
| 12 | GARCH(1,1) variance recursion (`garch11_variance_recursion`) | ✅ IMPLEMENTED | Measured: 219ms→4.8ms first-call latency in a fresh process (not the ~300–500ms estimate); steady-state (warm numba) is actually 0.8× — i.e. C++ is slightly *slower* once numba is JIT-compiled. The entire value of this port is the cold-start column. |
...
| `run_strategy` (n=2000) | **~1.0×** (68.1ms → 67.7ms) | See discussion below — pandas wrapper overhead, not the kernel, dominates at this size. |
...
| `garch11_variance_recursion` (n=2000, warm) | **0.8×** (10.8ms → 12.9ms, C++ slightly slower) | The cold-start column below is the entire point of this port. |
...
**Two honest surprises, called out rather than smoothed over:**

- **`run_strategy` measures ~1.0× end-to-end**, not the documented 3–8×. `bench_backtest.cpp`'s native-only numbers (0.017ms at n=2000) confirm the raw kernel genuinely is fast in isolation — the ~68ms measured through the Python wrapper is almost entirely pandas/DataFrame construction, Series indexing, and result assembly, not the kernel call itself. The batch grid kernel (`batch_run_strategy`) is where this actually pays off, since it eliminates *per-combination* Python re-entry, not per-call wrapper overhead — that comparison wasn't independently re-measured this pass (see item 6's "not independently re-measured" note above).
- **OpenMP's measured speedup for `simulate_forward_paths` is ~1.4×** on this 16-core machine (405.8ms @ 1 thread vs. 293.4ms unconstrained, `n_simulations=200000`, measured via separate process invocations with `OMP_NUM_THREADS` set before each — an in-process env-var toggle does *not* work, since the OpenMP runtime reads it once at thread-pool creation, not on every parallel region). Not the near-linear-with-cores scaling the per-path independence would suggest in theory. MSVC's OpenMP support here is version 2.0 (an older spec), and each simulated path allocates its own small buffer inside the parallel loop, which is a plausible source of allocator contention across threads at this problem size — a real, positive, and reproducible number, just not a dramatic one on this build.

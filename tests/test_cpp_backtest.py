...
class TestCppBatchRunStrategy:
    ...
    def _signals_matrix(self, n: int, num_tests: int, seed: int = 0) -> np.ndarray:
        ...
        rows = [
            np.sign(rng.standard_normal(n)).astype(np.float64) for _ in range(num_tests)
        ]
...
class TestBacktestGridCppPath:
    ...
    def test_cpp_grid_matches_python_fallback(self):
        ...
        cpp_result = backtest_grid(
            prices, strategy="sma_crossover", param_grid=grid, n_workers=1
        )

        # Python fallback: patch HAS_CPP to False inside engine module
        with patch.object(eng, "HAS_CPP", False), patch.object(eng, "_cpp_core", None):
            py_result = backtest_grid(
                prices, strategy="sma_crossover", param_grid=grid, n_workers=1
            )

        assert len(cpp_result) == len(py_result)
        cpp_sorted = cpp_result.sort_values(["fast_period", "slow_period"]).reset_index(
            drop=True
        )
        py_sorted = py_result.sort_values(["fast_period", "slow_period"]).reset_index(
            drop=True
        )

...
from standard_quant_tools.error import (
    APIError, DataNotFoundError, InvalidSymbolError,
)
...
class TestGetOHLCV:
    def test_returns_dataframe_with_correct_columns(self, patched_factory, sample_ohlcv):
...
class TestGetFinancialRatios:
    ...
    def test_ratios_are_numeric_or_none(self, patched_factory):
        ...
        for field in ('forward_pe', 'trailing_pe', 'price_to_book',
                      'debt_to_equity', 'return_on_equity', 'profit_margins',
                      'dividend_yield'):

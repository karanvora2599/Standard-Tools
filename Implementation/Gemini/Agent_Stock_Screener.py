"""
Agentic Stock Screener — Google Gemini 2.0 Flash.
Gemini filters a universe, profiles every passer, and produces a ranked watchlist.
"""
...
Step 1 — Screen
  Call run_screener with filters:
...
Step 2 — Enrich each passing stock
  For each stock: call get_technical_analysis (all indicators) and
  analyze_stock_risk (period="1y") for alpha, beta, Sharpe, VaR.

Step 3 — Size each position
  For each stock, call get_position_size:
...
if __name__ == "__main__":
    log_file = setup_logging("agent_stock_screener_gemini")

    _header("Agentic Stock Screener — Gemini 2.0 Flash")

"""
Agentic Risk Monitor — Google Gemini 2.0 Flash.
Gemini audits a portfolio's risk profile and fires alerts for threshold breaches.
"""
...
PORTFOLIO  = {"tickers": ["AAPL", "MSFT", "GOOGL", "NVDA", "TSLA"],
...
2. Risk decomposition check
   ...
   - PC1 explains > {pc1_pct}% of total variance                      → WARNING
...
3. Per-asset check
   ...
   - sharpe_ratio < 0                                                  → ALERT

4. Technical regime check
   For each holding, call get_technical_analysis. Flag if:
   - rsi_14 > {rsi_ob}   → WARNING (overbought)
   - rsi_14 < {rsi_os}   → WARNING (oversold)
...
6. Write the risk report
   ## RISK SUMMARY  (HEALTHY / AT RISK / CRITICAL)
   ## ALERTS
   ## WARNINGS
   ## OK
   ## RECOMMENDATIONS

Always cite exact metric values and the threshold breached.""".format(
...
- Daily VaR95 limit : {THRESHOLDS['portfolio_var95_pct']}%
- Min Sharpe        : {THRESHOLDS['sharpe_ratio_min']}
- Max asset risk    : {THRESHOLDS['single_asset_risk_pct']}%
- PC1 limit         : {THRESHOLDS['pc1_variance_pct']}%
- Beta limit        : {THRESHOLDS['beta_max']}
- RSI overbought    : {THRESHOLDS['rsi_overbought']}
- RSI oversold      : {THRESHOLDS['rsi_oversold']}
...
if __name__ == "__main__":
    log_file = setup_logging("agent_risk_monitor_gemini")

    _header("Agentic Risk Monitor — Gemini 2.0 Flash")

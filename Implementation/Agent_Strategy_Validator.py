"""
Agentic Strategy Validator — Claude runs a candidate strategy through a
full pre-deployment validation pipeline instead of trusting a single
backtest number: does it actually work, is the edge statistically real
(not just the best of a lucky parameter search), is the underlying data
trustworthy, and would the strategy actually be tradeable at the account
size and liquidity available.

The agent:
  1. Runs the candidate strategy compactly (run_backtest_compact) for a
     summary/cost/risk breakdown without pulling the full equity curve
  2. Checks robustness via a parameter grid search (get_robustness_diagnostics)
     — Deflated Sharpe Ratio and parameter sensitivity, not just whichever
     combination happened to backtest best
  3. Checks the underlying OHLCV data quality (get_data_quality_report)
     before trusting either of the above — a backtest is only as good as
     the data under it
  4. Simulates deploying the (now-validated) strategy as a real, shared-cash
     multi-asset portfolio with realistic transaction costs and leverage
     limits (run_portfolio_simulation) — not each ticker getting its own
     independent capital
  5. Checks the target account size against each ticker's own liquidity
     (get_capacity_report) — a backtest that looks great on paper can be
     untradeable at size

A sixth tool, run_pair_trade_backtest, is also available for validating a
cointegrated pairs trade (given a hedge ratio, e.g. from run_cointegration_test)
with the same shared-cash, cost-aware rigor as run_portfolio_simulation —
not exercised by the demo request below, but described in the system prompt
so the agent can use it if asked about a pairs strategy instead.
"""
...
if __name__ == "__main__":
    log_file = setup_logging("agent_strategy_validator")

    _header("Agentic Strategy Validator — Claude Haiku")

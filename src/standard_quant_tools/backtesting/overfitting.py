...
def _norm_ppf(p: float) -> float:
    ...
    Accurate to about 1.15e-9 over the whole range, which is far beyond what
    any of these statistics need. Written out rather than imported because
    scipy is not a declared dependency of this package.
    """
    if not 0.0 < p < 1.0:
        raise ValidationError(f"_norm_ppf: p must be in (0, 1), got {p!r}")
    a = [

    ...
    double* SQT_RESTRICT       out)
{
    ...
    out[0] = (mean < kMinSigma2) ? kMinSigma2 : mean;

    for (std::size_t t = 1; t < n; ++t) {
        const double s2 = omega + alpha * resid_sq[t - 1] + beta * out[t - 1];
        out[t] = (s2 < kMinSigma2) ? kMinSigma2 : s2;
    }
}
    ...
    double        beta)
{
    ...
    garch11_variance_recursion_into(resid_sq, n, omega, alpha, beta, sigma2.data());
    return sigma2;
}

...
def stack_long(
    ...
    [date, entity, <feature_ids>, target] — rows with any NaN feature or
    target dropped (unavoidable at the start of each feature's lookback
    window and the end of the target's forward horizon).
    """
    frames = []
    ...
    feature_cols = [c for c in long_panel.columns if c not in ("date", "entity", "target")]

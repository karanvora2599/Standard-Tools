...
def _norm_ppf(p: float) -> float:
    """
    Standard normal inverse CDF (quantile function) via Acklam's rational
    approximation (public-domain algorithm, accurate to ~1e-9) — avoids a
    hard scipy dependency for a function only needed here.
    """
    if p <= 0.0:
        return float("-inf")
    if p >= 1.0:
